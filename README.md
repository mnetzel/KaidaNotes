...
| `js/address-state.js` | Save the current tab’s composition in its URL, ignoring stale asynchronous writes |
...
The address fragment is the source of saved state. Every edit, undo/redo and Clear All updates it using history.replaceState, without reloading or adding history entries. Tabs are independent, including when they start from the same link. A bare app URL starts an empty composition. The editor no longer reads or writes localStorage; the unused legacy persistence module remains only for old compatibility tests. Existing localStorage is untouched. Undo history and selection remain transient. A brief saving status is shown while compression runs; stale completions cannot overwrite newer edits. If URL encoding fails or exceeds its size limit, an unsaved status is shown and navigation warns rather than silently discarding unsaved edits.
...
Incoming links load automatically and remain in the address. Edits continuously replace that address with the current snapshot; copying the browser address shares the latest saved state. An unreadable link leaves the current editor intact and reports the problem in the save status. Compatible older payloads are normalized by the importer without requiring an exact field-for-field match.
...
Presentation View offers **Share image** and **Save image**. Opening the view prepares a PNG of the full notation, block timeline (when recorded) and notes, excluding controls. File sharing uses the native share sheet (choose WhatsApp); browsers without file sharing download the PNG instead. The image is prepared before the share tap to preserve iOS user activation. Export draws text, annotations and timeline primitives directly to Canvas with the loaded Iosevka fonts. Coordinates are relative to the full sheet, independent of modal position, preview scale, scrolling and orientation. It does not rasterize HTML inside SVG, which can render blank or clipped in Safari. Composition data is unchanged and image generation stays in the browser.

Optional browser regression: with the local server running and Playwright (Chromium and WebKit) installed, run `node tests/browser/presentation-framing.mjs`. `PLAYWRIGHT_MODULE` can point to an external Playwright module; `KAIDA_TEST_URL` overrides the local app URL. This checks short/long sheets, complete top-to-bottom content, and equivalent pixels even when the preview is scaled and moved entirely outside the viewport.
