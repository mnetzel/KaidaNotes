...
Presentation View offers **Share image** and **Save image**. Opening the view prepares a PNG of the full notation, block timeline (when recorded) and notes, excluding controls. File sharing uses the native share sheet (choose WhatsApp); browsers without file sharing download the PNG instead. The image is prepared before the share tap to preserve iOS user activation. Export uses the local, MIT-licensed html-to-image distribution and embedded Iosevka fonts; composition data is unchanged and image generation stays in the browser.
